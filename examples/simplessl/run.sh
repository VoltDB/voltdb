...
function clean() {
    rm -rf voltdbroot log client/simplessl/*.class *.log
}

# remove everything from "clean" as well as the jarfiles
function cleanall() {
    clean
    rm -rf simplessl-client.jar
}

# compile the source code for procedures and the client into jarfiles
function jars() {
    # compile java source
    javac -classpath $CLIENTCLASSPATH client/simplessl/*.java
    # build procedure and client jars
    jar cf simplessl-client.jar -C client simplessl
    # remove compiled .class files
    rm -rf client/simplessl/*.class
}
