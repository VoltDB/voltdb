            ...
            $.each(connection.Metadata['DATABASE_STATUS'].serverDetails, function(k, v) {
                //display the key and value pair
                    if(v[Object.keys(v)[0]].status!="running"){
                        ...
                        '<li class="pullback"><span class="redC"><strong>X</strong></span> '+ Object.keys(v)[0] +'<span class="redC"> <strong>(too much clock skew)</strong></span></li></ul>'
                    }
                    else
                    {
                        msg = msg + '<ul class="listStalled">' +
                        '<li class="pullback"><span class="redC"></span>'+  Object.keys(v)[0]+'</li></ul>'
                    }
            });
            ...
            VdmService.GetDatabaseStatus(function(connection){
                ...
                if($("#startOption option:selected").val() == 1){  //Start Cluster
                      VdmService.StartCluster(function(connection){
                        if(connection.Metadata['VDM_START_CLUSTER'].statusstring.indexOf('There were errors starting servers') >= 0){
                           $(".loadingStart").hide();
                            $(".startFail").show();
                            $(".containerFailed").html('');
                            $(".containerFailed").html('The cluster failed to start because of ' + connection.Metadata['VDM_START_CLUSTER'].statusstring );
                     ...
                     VdmService.RecoverCluster(function(connection){
                        if(connection.Metadata['VDM_RECOVER_CLUSTER'].statusstring != 'Query timeout.'){
                         $(".startFail").hide();
                         $(".clusterBtnWrap").hide();
                         $(".databaseRg").show();
                         $(".loadingStart").hide();
