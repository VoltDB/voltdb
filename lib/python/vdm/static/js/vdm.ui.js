...
//            checkDuplicateReplicationListener: 'This port already exists',
    ...
    if (VdmUI.getCurrentServerCookie() == 0)
    {
      ...
      var serverData = {
            ...
            VdmService.GetServer(function(connection){
                ...
                $("#txtVoltdbRootDir").val(serverInfo['voltdbroot'])
                $("#txtSnapshotDir").val(serverInfo['snapshot'])
                $("#txtExportOverflowDir").val(serverInfo['exportoverflow'])
                $("#txtCommandLogDir").val(serverInfo['commandlog'])
                $("#txtCommandLogSnapDir").val(serverInfo['commandlogsnapshots'])
                $("#txtDrOverflow").val(serverInfo['droverflow'])
...
//                serverId = VdmUI.getCurrentServerCookie();
//
//                if (serverId == 0){ //update default port settings
//                    serverInfo = {
//                        data:{
       ...
       if (!$("#"+form).valid()) {
            ...
            if (serverId == 0){ // Updating default Settings
                if(item == "voltdbrootDir"){
                    serverInfo = {
                        data: {
                                voltdbroot: clusterDirectoryObjects.txtVoltdbRootDir.val()
                              }
                    }
                }
                else if(item == "snapshotDir"){
                    serverInfo = {
                        data: {
                                snapshot: clusterDirectoryObjects.txtSnapshotDir.val()
                              }
                    }
                }
                else if(item == "exportOverflowDir"){
                    serverInfo = {
                        data: {
                                exportoverflow: clusterDirectoryObjects.txtExportOverflowDir.val()
                              }
                    }
                }
                else if(item == "commandlogDir"){
                    serverInfo = {
                        data: {
                                commandlog: clusterDirectoryObjects.txtCommandLogDir.val()
                              }
                    }
                }
                else if(item == "commandlogSnapDir"){
                    serverInfo = {
                        data: {
                                commandlogsnapshots: clusterDirectoryObjects.txtCommandLogSnapDir.val()
                              }
                    }
                }
                 else if(item == "drOverflow"){
                    serverInfo = {
                        data: {
                                droverflow: clusterDirectoryObjects.txtDrOverflow.val()
                              }
                    }
                }

                VdmService.SaveDefaultPath(function(connection){
                    VdmUI.displayDefaultPaths();
                }, serverInfo);
            }
            else{
...
//            }
        ...
        this.displayDefaultPaths = function(){
            VdmService.GetDefaultPath(function(connection){

                 var arr =  connection.Metadata['DEFAULT_PATH'];
                   $("#txtVoltdbRootDir").val(arr.defaultPath[0]['voltdbroot'])
                    $("#txtSnapshotDir").val(arr.defaultPath[0]['snapshot'])
                    $("#txtExportOverflowDir").val(arr.defaultPath[0]['exportoverflow'])
                    $("#txtCommandLogDir").val(arr.defaultPath[0]['commandlog'])
                    $("#txtCommandLogSnapDir").val(arr.defaultPath[0]['commandlogsnapshots'])
                    $("#txtDrOverflow").val(arr.defaultPath[0]['droverflow'])
                })
        }
            ...
            $('tr.diskLimitParentProp > td:first-child' || 'tr.diskLimitParentProp > td:fourth-child')
             ...
             $('.defaultServ').on('change', function(){
                                ...
                                if (value == 0){ //Default settings
                                    ...
                                    var serverInfo = $(this).find(':selected').data('info');
