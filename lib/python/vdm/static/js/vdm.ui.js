         ...
         if (connection.Metadata['DATABASE_STATUS'].status[0].status=="stopped")
            {
                $.each(connection.Metadata['DATABASE_STATUS'].serverDetails, function(k, v) {
                                if(v[Object.keys(v)[0]].hasOwnProperty('details') && v[Object.keys(v)[0]].details != "")
                                  {
                                   if(v[Object.keys(v)[0]].details.indexOf('@Shutdown') < 0)
                ...
                if (failedOrStalled && !VdmUI.isStartServer)
                {
                    ...
                    $.each(connection.Metadata['DATABASE_STATUS'].serverDetails, function(k, v) {
                        //display the key and value pair
                            ...
                            '"'+v[Object.keys(v)[0]].details+'"</div>      <div class="modal-footer"><button id="" type="button" class="btn" data-dismiss="modal">OK</button>' +
                            ...
                            $.each(connection.Metadata['DATABASE_STATUS'].serverDetails, function(k, v) {
                                //display the key and value pair
                                    if(v[Object.keys(v)[0]].status!="running"){
                                        var ip = Object.keys(v)[0];
                                            msg = msg + '<ul class="listStalled">' +
                                        '<li class="pullback"><span class="redC"><strong>X</strong></span> '+ ip +'<span class="redC left-space"> <a href="javascript:void(0);" data-toggle="modal" data-target="#plusError'+ k +'" >error</a></span><div id="plusError'+ k +'" class="errorLogPopup modal fade in" role="dialog">' +
                                        '<div class="modal-dialog modal-dialog-center modal-add-user"><div class="modal-content"><div class="modal-header">' +
                        '<h4 class="modal-title" id="errorHeader">Error Detail</h4></div><div class="modal-body">' +
                        '"'+v[Object.keys(v)[0]].details+'"</div>      <div class="modal-footer"><button type="button" id="btnRecoverOk" class="btn" data-dismiss="modal">OK</button>' +
                      '</div></div></div></div></li></ul>'
                    }
                    else
                    {
                        ...
                        $.each(startStatus.serverDetails, function(k, v) {
                            //display the key and value pair
                                if(v[Object.keys(v)[0]].status=="stopped"){
                                    ...
                                    '<div class="modal-dialog modal-dialog-center modal-add-user"><div class="modal-content"><div class="modal-header">' +
                    '<h4 class="modal-title" id="errorHeader">Error Detail</h4></div><div class="modal-body">' +
                    '"'+v[Object.keys(v)[0]].details+'"</div>      <div class="modal-footer"><button id="btnFailedOk" type="button" class="btn" data-dismiss="modal">OK</button>' +
                  '</div></div></div></div></li></ul>'
                }
                else
                {
...
//                }
    ...
    $('#btnStopCluster').on('click', function(){
        ...
        VdmService.StopCluster(function(connection){
            if(connection.Metadata['VDM_STOP_CLUSTER'].statusstring != 'Shutdown request sent successfully'){
              //try killing cluster
                   VdmService.StopCluster(function(connection){
