                ...
                if(dbIdList.length != 0 ){
                    ...
                    VdmService.GetDatabaseStatus(function(connection){
                        ...
                        for(var i =0; i< database_server_status.length; i++){
                            ...
                            serverInitStateArray[key] = database_server_status[i][key].isInitialized
                        }
                        if(memberList.members != undefined){
                            ...
                            memberList.members.forEach(function (info) {
                                ...
                                if (server_state.toLowerCase() == 'running'){
                                    ...
                                    htmlList += '<tr> ' +
                                                '<td id="tdHostname_' +hostId+ '"> ' +
                                                '<span class="redDot serverListIcons" title="Server is running "></span> ' +
                                                '<span class="clsServerListGreen">' + displayName + '</span> ' +
                                                '</td> ' +
                                                '<td> ' +
                                    ...
                                    htmlList += '<tr> ' +
                                                ...
                                                '<span class="redDot serverListIcons" title="Server is running "></span> ' +
                                                '<div class="dropdown serverListDropdown"> ' +
                                                ...
                                                '</div> ' +
                                                '<ul data-id="' + hostId + '"  data-id="' + hostId + '" data-info=\''+ infos +'\' class="dropdown-menu" aria-labelledby="dropdownMenu1" > ' +
                                                '<li class="btnUpdateServer"><a data-toggle="modal" data-target="#addServer"><span class="glyphicon glyphicon-pencil"></span>Edit</a></li> ' +
                                                '<li class="btnDeleteServer"><a data-toggle="modal" data-target="#deleteConfirmation"><span class="glyphicon glyphicon-trash redC"></span>Delete</a></li> ' +
                                                '</ul></div> ' +
                                                '</td> ' +
                                                '<td> ' +
                                                '<a class="btnServerLog" href="javascript:void(0);"data-toggle="modal" data-target="#divServerLogPopup" >' +
                                                ...
                                                '<label style="display:none">' + logMsg + '</label> ' +
                                                '</td>' +
                                                '<td data-id="' + hostId + '" data-initialized="' + serverInitStateArray[hostName] + '" data-hostcount= "'+ memberCount +'" ' +
                                                'data-ip="' + hostName + '"> ' +
                        ...
                        $('.btnStopServer').on('click', function(){
                            ...
                            VdmService.StopServer(function(connection){
                                ...
                                if(connection.Metadata['VDM_STOP_SERVER'].statusString.indexOf('shutdown successfully') < 0){
                                    //try killing server
                                    VdmService.KillServer(function(connection){
                                         if(connection.Metadata['VDM_KILL_SERVER'].statusString.indexOf('success') <= -1){
                        ...
                        $('.btnStartServer').on('click', function(e){
                            ...
                            if($(this).hasClass('startServerEnabled')){
                                ...
                                result = $(this.parentElement).data('initialized')
                                if (result){
                                    ip_add = VdmUI.GetRunningServerIp(serverStateArray)
                                    if(ip_add != ''){
                                        VdmService.GetSystemInformation(function(connection){
                                            if(connection.Metadata['SYSTEM_INFORMATION'] != null){
                                                var result = VdmUI.CheckHostCount(connection.Metadata['SYSTEM_INFORMATION'], serverInitStateArray)
                                                if (result){
                                                    VdmService.GetStatisticsInformation(function(connection){
                                                        ...
                                                        && connection_data['results'][0].data.length != 0){
                                                            ...
                                                            if(missing_replica != undefined && missing_replica > 0){
                                                                ...
                                                                VdmUI.DisplayError('Couldn\'t start the given server.')
                                                            }
                                                        } else {
                                                            VdmUI.DisplayError('Couldn\'t start the server.')
                                                        }
                                                    },{'serverIp': ip_add});
                                        ...
                                        VdmUI.DisplayError('Database does\'t seem to be working.')
                                    }
                                } else {
...
//                $("#txtJdbcUrlValue").val("")
        ...
        this.GetDatabaseStatus = function(){
            $('#btnStopCluster').show()
