...
//            checkDuplicateReplicationListener: 'This port already exists',
    ...
    $.validator.addMethod(
        "checkDuplicateHttpListener",
        function (value) {
           return CheckPort(value, "httplistener", VdmUI.httplistenerToUpdate)
        },
        "Port already exists."
    );
    ...
    $.validator.addMethod(
        "checkDuplicateAdminListener",
        function (value) {
           return CheckPort(value, "adminlistener", VdmUI.adminlistenerToUpdate)
        },
        "Port already exists."
    );
    ...
    $.validator.addMethod(
        "checkDuplicateInternalListener",
        function (value) {
           return CheckPort(value, "internallistener", VdmUI.internallistenerToUpdate)
        },
        "Port already exists."
    );
    ...
    $.validator.addMethod(
        "checkDuplicateZookeeperListener",
        function (value) {
           return CheckPort(value, "zookeeperlistener",VdmUI.zookeeperlistenerToUpdate)
        },
        "Port already exists."
    );
    ...
    $.validator.addMethod(
        "checkDuplicateClientListener",
        function (value) {
           return CheckPort(value, "clientlistener", VdmUI.clientlistenerToUpdate)
        },
        "Port already exists."
    );
    ...
    $.validator.addMethod(
        "checkDuplicateReplicationListener",
        function (value) {
           return CheckPort(value, "replicationlistener",VdmUI.replicationlistenerToUpdate)
        },
        "Port already exists."
    );

    function CheckPort(value, option, valueToUpdate){
            ...
            for(var i=0; i<= server.length -1; i++)
            {
                ...
                {
                    portList.push(server[i][option])
                }

            }

            if (VdmUI.isServerCreate == false) {
                    if ($.inArray(value, portList) != -1) {
                        if (value == valueToUpdate)
                            return true;
                        if (value == ""){
                            $.validator.messages.checkDuplicateHttpListener = "Default port already exists";
                            $.validator.messages.checkDuplicateAdminListener = "Default port already exists";
                            $.validator.messages.checkDuplicateInternalListener = "Default port already exists";
                            $.validator.messages.checkDuplicateZookeeperListener = "Default port already exists";
                            $.validator.messages.checkDuplicateClientListener = "Default port already exists";
                            $.validator.messages.checkDuplicateReplicationListener = "Default port already exists";
                        }
                        else
                        {
                           $.validator.messages.checkDuplicateHttpListener = "This port already exists.";
                            $.validator.messages.checkDuplicateAdminListener = "This port already exists.";
                            $.validator.messages.checkDuplicateInternalListener = "This port already exists.";
                            $.validator.messages.checkDuplicateZookeeperListener = "This port already exists.";
                            $.validator.messages.checkDuplicateClientListener = "This port already exists.";
                            $.validator.messages.checkDuplicateReplicationListener = "This port already exists.";
                            }
                        return false;
                    } else {
                        return true;
...
(function (window) {
    var iVdmUi = (function () {
        ...
        this.CurrentHttpPortList = [];
        this.CurrentClientPortList = [];
        this.CurrentAdminPortList = [];
        this.CurrentInternalPortList = [];
        this.CurrentZookeeperPortList = [];
        this.CurrentReplicationPortList = [];
        this.ServerList = {};
        ...
        this.displayServers = function(memberList){
            ...
            VdmService.GetDatabaseList(function(connection){
                ...
                if(dbIdList.length != 0 ){
                    ...
                    VdmUI.CurrentHttpPortList = [];
                    VdmUI.CurrentClientPortList = [];
                    VdmUI.CurrentAdminPortList = [];
                    VdmUI.CurrentInternalPortList = [];
                    VdmUI.CurrentZookeeperPortList = [];
                    VdmUI.CurrentReplicationPortList = [];
                    VdmUI.ServerList = []
                    if(memberList.members != undefined){
                        memberList.members.forEach(function (info) {
                            ...
                            VdmUI.CurrentHttpPortList.push(info["http-listener"])
                            VdmUI.CurrentClientPortList.push(info["client-listener"])
                            VdmUI.CurrentAdminPortList.push(info["admin-listener"])
                            VdmUI.CurrentInternalPortList.push(info["internal-listener"])
                            VdmUI.CurrentZookeeperPortList.push(info["zookeeper-listener"])
                            VdmUI.CurrentReplicationPortList.push(info["replication-listener"])
