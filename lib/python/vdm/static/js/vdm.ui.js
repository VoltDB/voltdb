...
var loadPage = function() {
    ...
    $( "#selectAuthProtocol" ).change(function() {
        VdmUI.addRemoveAuthkeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectAuthProtocol.val())
    });

    $( "#selectPrivacyProtocol" ).change(function() {
        VdmUI.addRemovePrivacyKeyValidation(clusterObjects.txtUsername.val(), clusterObjects.selectPrivacyProtocol.val())
    });
    ...
    $('#chkSNMP').on('ifClicked', function(event){
        ...
        if(isEnabled){
            $('#txtTarget').rules('add', {
                required: true,
                ipPortRegex: /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/,
                messages: {
...
//                $("#txtJdbcUrlValue").val("")
            ...
            VdmService.GetDeployment(function(connection){
                ...
                if(deploymentInfo['deployment']['snmp'] != undefined && !$.isEmptyObject(deploymentInfo['deployment']['snmp'])){
                    if(deploymentInfo['deployment']['snmp']['enabled'] == true){
                        ...
                        VdmUI.addRemoveAuthkeyValidation(deploymentInfo['deployment']['snmp'].username, deploymentInfo['deployment']['snmp'].authprotocol)
                        VdmUI.addRemovePrivacyKeyValidation(deploymentInfo['deployment']['snmp'].username, deploymentInfo['deployment']['snmp'].privacyprotocol)
                    } else {
                        $('#txtTarget').rules('remove');
                        $('#txtAuthKey').rules('remove');
                        $('#txtPrivacyKey').rules('remove');
        ...
        this.addRemoveAuthkeyValidation = function(username, authProtocol){
            var isEnabled = false;
            if($($('#chkSNMP').parent()).hasClass('checked'))
                isEnabled = true;
            if(isEnabled && authProtocol != 'NoAuth'){
                $('#txtAuthKey').rules('remove' );
        ...
        this.addRemovePrivacyKeyValidation = function(username, privacyProtocol){
            var isEnabled = false;
            if($($('#chkSNMP').parent()).hasClass('checked'))
                isEnabled = true;
            if(isEnabled  && privacyProtocol != 'NoPriv'){
                $('#txtPrivacyKey').rules('remove');
                if(username != undefined && username != ''){
