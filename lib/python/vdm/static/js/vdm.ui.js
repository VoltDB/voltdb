...
//            $('.child-row-5').find('td:first-child > a').attr('class',childRowClass)

//            if (!$('.child-row-5').find('td:first-child > a').hasClass('glyphicon-triangle-bottom'))
//            {
//                $('tr[class^=childprop-row-41]').hide().children('td');
//            }
...
//                    }
        ...
        this.SyncVdmConfiguration = function() {
            VdmService.GetServerIp(function(connection){
                if(!connection['Metadata']['VDM_GET_IP'].hasOwnProperty('ip_address')){
                   $('#errorMsg').html('Unable to get server IP address.')
                   $('#errorDialog').modal('show');
                } else {
                    var ip_address = connection.Metadata['VDM_GET_IP'].ip_address
                    VdmService.GetServerList(function(connection){
                        serverList = connection.Metadata['SERVER_LISTING'];
                        serverList.servers.forEach(function (info) {
                            var hostName = info["hostname"];
                            var serverName = info["name"]
                            var vdmData = {'serverIp': hostName, 'data':{'ip_address':ip_address}}
                            VdmService.SyncVdmConfiguration(function(connection){
                                if(!connection['Metadata']['VDM_SYNC'].hasOwnProperty('status')){
                                   $('#errorMsg').html('Unable to sync clusters.')
                                   $('#errorDialog').modal('show');
                                }
                            }, vdmData);
                        });
