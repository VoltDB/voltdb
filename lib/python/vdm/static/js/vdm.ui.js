...
var loadPage = function() {
    ...
    var validationRules = {
        ...
        ConnectionSourceServerMessages:{
            required: "This field is required when connection source in enabled.",
        },
        DrPortRules: {
            min: 0,
            max: 65535,
    ...
    clusterObjects = {
        ...
        chkConnectionSourceOnOff: $('#chkConnectionSourceOnOff'),
        chkConnectionSourceOnOffValue: $('#chkConnectionSourceOnOffValue'),
        lblDrPort: $('#lblDrPort'),
        rowDrPort: $('#rowDrPort'),
    }
 ...
 var dbData = {
    ...
    VdmService.GetDatabaseStatus(function(connection){
            ...
            if (connection.Metadata['DATABASE_STATUS'].status=="stopped")
            {
                $(".databaseRg").hide();
                $(".clusterBtnWrap").show();
                $(".stalledState").hide();
            ...
            else if (connection.Metadata['DATABASE_STATUS'].status=="running")
            {
                $(".databaseRg").show();
                $(".clusterBtnWrap").hide();
                $(".stalledState").hide();
            ...
            else if (connection.Metadata['DATABASE_STATUS'].status=="stalled")
            {
                $(".stalledState").show();
                console.log(VdmUI.CurrentServerList);
                var msg = '<p>Cluster startup did not succeed on all servers.</p>';
                for(var i=0 ; i<=VdmUI.CurrentServerList.length-1; i++)
                {
                   msg = msg + '<ul class="listStalled">' +
                        '<li class="pullback"><span class="redC"><strong>X</strong></span> '+ VdmUI.CurrentServerList[i]+'<span class="redC"> <strong>(too much clock skew)</strong></span></li></ul>'
                }
    ...
    $('#btnCreateServerOk').on('click', function(e){
        ...
        if(!VdmUI.isTestServer){
            VdmService.GetVdmStatus(function(connection){
                ...
                if(connection['Metadata']['VDM_STATUS'].hasOwnProperty('vdm')){
                    result = connection['Metadata']['VDM_STATUS']['vdm'].running;
                    if(Boolean(result)){
                        var serverInfo ={
                            data:{
                                "name" : $('#serverName').val(),
    ...
    SaveImport = function(){
        ...
        for(var i=0;i<= VdmUI.importPropertyList.length -1;i++)
        {
            deploymentInfo['data']['import']['configuration'].push({
                ...
                format: VdmUI.importPropertyList[i].importconnectorclass==undefined?"":VdmUI.importPropertyList[i].importconnectorclass,
                property: []
            });
...
//                }
    ...
    $('#chkConnectionSourceOnOff').iCheck({
        checkboxClass: 'icheckbox_square-aero customCheckbox',
        increaseArea: '20%'
    })

    $('#chkConnectionSourceOnOff').on('ifChanged', function () {
        VdmUI.showHideConnectionSource();
    });
    ...
    var SaveDrInfo = function(){
        ShowSavingStatus();
        var dr_value = {}
        if(clusterObjects.chkDrOnOffValue.text() == 'On') {
            var masterCluster = clusterObjects.txtDatabase.val();
            var connection_source = {}

            if(clusterObjects.chkConnectionSourceOnOffValue.text() != 'Off'){
                connection_source = {
    ...
    $('#btnStartCluster').on('click', function(){
        ...
        VdmService.GetDatabaseStatus(function(connection){
            ...
            VdmService.StartCluster(function(connection){
                if(connection.Metadata['VDM_START_CLUSTER'].statusstring != 'Query timeout.'){
                 $(".stalledFail").hide();
                 $(".clusterBtnWrap").hide();
                 $(".databaseRg").show();
                 $(".loadingStart").hide();
                    ...
                    $(".loadingStart").hide();
                    $(".stalledFail").show();
                    $(".containerStalled").html('');
                    $(".containerStalled").html('The cluster failed to start because of ' + connection.Metadata['VDM_START_CLUSTER'].statusstring );
                }
            }, dbInfo);
            ...
            else if (connection.Metadata['DATABASE_STATUS'].status=="stalled")
            {
                ...
                {
                   msg = msg + '<ul class="listStalled">' +
                        '<li>'+VdmUI.CurrentServerList[i]+'</li>' +
                        '<li class="pullback"><span class="redC"><strong>X</strong></span> Pullux<span class="redC"> <strong>(too much clock skew)</strong></span></li></ul>'
                }
                $(".containerStalled").html(msg);
                $(".loadingStart").hide();
                $(".clusterBtnWrap").hide();
    ...
    $('#btnStopCluster').on('click', function(){
        ...
        VdmService.StopCluster(function(connection){
            if(connection.Metadata['VDM_STOP_CLUSTER'].statusstring == 'success'){
                $('#btnStartCluster').show()
                $('#btnStopCluster').hide()
            }else{
                $('#errorMsg').html('There were errors stopping servers.')
                $('#errorDialog').modal('show');
            }
        }, dbInfo);
    })
        ...
        VdmService.UploadDeploymentXml(function(connection){
            if(connection.Metadata['UPLOAD_DEPLOYMENT_CONFIG'].hasOwnProperty('status')){
                if(connection.Metadata['UPLOAD_DEPLOYMENT_CONFIG']['status'].toString().toLowerCase() == 'success'){
                    ...
                    $('#errorMsg').html('Unable to upload file to server. ' + connection.Metadata['UPLOAD_DEPLOYMENT_CONFIG']['error'])
                    $('#errorDialog').modal('show');
                }
                ...
                if(connection.Metadata['UPLOAD_DEPLOYMENT_CONFIG'].hasOwnProperty('error')){
                    ...
                    $('#errorMsg').html('Unable to upload file to server. ' +  connection.Metadata['UPLOAD_DEPLOYMENT_CONFIG']['errors'][0])
                    $('#errorDialog').modal('show');
                } else {
        ...
        this.displayDeployment = function(deployment){
            ...
            if(deployment['deployment']['dr'] != null && !$.isEmptyObject(deployment['deployment']['dr'])){
                ...
                if(deployment['deployment']['dr'].connection != null && ! $.isEmptyObject(deployment['deployment']['dr'].connection)){
                    ...
                    $('#onOffConnectionIcon').removeClass('offIcon')
                    $('#onOffConnectionIcon').addClass('onIcon')
                    $('#chkDrOnOffConnectionVal').text('On')
                    ...
                    $('#onOffConnectionIcon').removeClass('onIcon')
                    $('#onOffConnectionIcon').addClass('offIcon')
                    $('#chkDrOnOffConnectionVal').text('Off')
...
//                $("#txtJdbcUrlValue").val("")
        ...
        this.showHideDrProperties = function(){
            if($('#chkDrOnOffVal').text().toLowerCase().trim() == 'on'){
                if(!VdmUI.isConnectionSourceDefined){
                    $('.clsDrEnabled').hide();
                    $('.clsServerList').hide()
                }else{
                    $('.clsDrEnabled').show();
                ...
                $('.clsDrType').show();
                if(clusterObjects.lblDrPort.text() == ''){
                    clusterObjects.rowDrPort.hide();
                }
            } else {
                $('.clsDrType').hide();
        ...
        this.showHideConnectionSource = function(){
            if($('#chkConnectionSourceOnOffValue').text().toLowerCase() == 'off'){
