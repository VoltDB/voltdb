    ...
    def test_validate_authkey_privacykey(self):
        ...
        if value:
            ...
            response = requests.put(dep_url,
                                    json=json_data, headers=headers)
            value = response.json()
            self.assertEqual(str(value['statusString']), "SNMP: Authkey must be of atleat 8 characters.")
            self.assertEqual(response.status_code, 200)
            ...
            response = requests.put(dep_url,
                                    json=json_data, headers=headers)
            value = response.json()
            self.assertEqual(str(value['statusString']), "SNMP: PrivacyKey must be of atleat 8 characters.")
            self.assertEqual(response.status_code, 200)
