...
def killProcess(p):
    "Kill all processes for this user named 'IntrinsicLockPatternTest'"
    ...
    javaprocs = [int(t[0]) for t in javaprocs if t[1].startswith("IntrinsicLockPatternTest")]
...
output = os.system("javac IntrinsicLockPatternTest.java")
if output == 0:
    print "Success"
else:
    print "Failed to compile reproducer."
    print "Check the output of \"javac IntrinsicLockPatternTest.java\" from your shell."
    sys.exit(-1)
...
def runTest(i):
    ...
    p = Popen("java IntrinsicLockPatternTest", shell=True, bufsize=0, stdout=PIPE)
