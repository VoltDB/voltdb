...
class Stats(object):
    ...
    def file_jira_issue(self, issue, DRY_RUN=False):
        ...
        note = 'http://junitstatsdb/adminer.php?server=junitstatsdb&username=qaquery&db=qa&sql=select+%2A+from+%60junit-test-failures%60+WHERE+name+%3D+%27'+issue['name']+'%27+and+job+%3D+%27'+job+'%27order+by+build+desc'
