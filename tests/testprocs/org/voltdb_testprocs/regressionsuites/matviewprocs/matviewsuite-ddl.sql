...
-- table and views for ENG-8641
CREATE TABLE ENG8641 (pid INTEGER NOT NULL, d1 INTEGER NOT NULL, d2 INTEGER NOT NULL, v1 INTEGER, v2 INTEGER NOT NULL);
PARTITION TABLE ENG8641 ON COLUMN pid;

CREATE INDEX IDX8641expR ON ENG8641 (d1, d2, abs(v1));
CREATE INDEX IDX8641d12 ON ENG8641 (d1, d2);
CREATE INDEX IDX8641 ON ENG8641 (d1, d2, v2);
CREATE INDEX IDX8641expL ON ENG8641 (d1+1, d2*2, v2);
CREATE INDEX IDX8641expLR ON ENG8641 (d1+1, d2*2, v2-1);
CREATE INDEX IDX8641CLT ON ENG8641 (d1, d2, v1) WHERE v1 < 4;
CREATE INDEX IDX8641CGT ON ENG8641 (d1, d2, v1) WHERE v1 > 4;
CREATE INDEX IDX8641NG1 ON ENG8641 (v1);
CREATE INDEX IDX8641NG2 ON ENG8641 (v2);

CREATE VIEW VENG8641 (d1, d2, cnt, vmin, vmax) AS
SELECT d1, d2, COUNT(*), MIN(v2) AS vmin, MAX(v2) AS vmax
FROM ENG8641 GROUP BY d1, d2;

CREATE VIEW VENG8641expL (d1, d2, cnt, vmin, vmax) AS
SELECT d1+1, d2*2, COUNT(*), MIN(v2) AS vmin, MAX(v2) AS vmax
FROM ENG8641 GROUP BY d1+1, d2*2;

CREATE VIEW VENG8641expR (d1, d2, cnt, vmin, vmax) AS
SELECT d1, d2, COUNT(*), MIN(abs(v1)) AS vmin, MAX(abs(v1)) AS vmax
FROM ENG8641 GROUP BY d1, d2;

CREATE VIEW VENG8641expLR (d1, d2, cnt, vmin, vmax) AS
SELECT d1+1, d2*2, COUNT(*), MIN(v2-1) AS vmin, MAX(v2-1) AS vmax
FROM ENG8641 GROUP BY d1+1, d2*2;

CREATE VIEW VENG8641C (d1, d2, cnt, vmin, vmax) AS
SELECT d1, d2, COUNT(*), MIN(v1) AS vmin, MAX(v1) AS vmax FROM ENG8641
WHERE v1 > 4 GROUP BY d1, d2;

CREATE VIEW VENG8641TwoIndexes (d1, d2, cnt, vmin, vmax) AS
SELECT d1, d2, COUNT(*), MIN(abs(v1)) AS vmin, MAX(v2) AS vmax FROM ENG8641
WHERE v1 > 4 GROUP BY d1, d2;

CREATE VIEW VENG8641NoGroup (cnt, vmin, vmax) AS
SELECT COUNT(*), MIN(v1) AS vmin, MAX(v2) AS vmax FROM ENG8641;
