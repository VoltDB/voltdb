...
-- Migrating indexes on migrating tables
CREATE TABLE MTEST_R1 (
   ID INT NOT NULL,
   WAGE        SMALLINT,
   V VARCHAR(32)
) USING TTL 2 MINUTES ON COLUMN ID
MIGRATE TO TARGET FOO; -- target does not have to exist

CREATE MIGRATING INDEX M_R1_INDEX1 ON MTEST_R1 ();
CREATE MIGRATING INDEX M_R1_INDEX2 ON MTEST_R1 () WHERE V IS NOT NULL;
CREATE MIGRATING INDEX M_R1_INDEX3 ON MTEST_R1 (ID) WHERE V IS NOT NULL;
CREATE MIGRATING INDEX M_R1_INDEX4 ON MTEST_R1 (ID, WAGE) WHERE V IS NOT NULL AND WAGE > 5000;
