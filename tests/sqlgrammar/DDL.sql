...
CREATE VIEW VR6 (TINY,
  ...
  SELECT TINY,
    MIN(ID), SUM(SMALL), COUNT(INT), MIN(BIG), MAX(NUM), SUM(DEC),
    MIN(VCHAR_INLINE), MIN(VCHAR_INLINE_MAX), MIN(VCHAR_OUTLINE_MIN), MIN(VCHAR), MIN(VCHAR_JSON), MIN(TIME)
    -- Will not work with SqlCoverage/PostgreSQL:
    , MIN(VARBIN), MIN(POINT), MIN(POLYGON)
    , MIN(IPV4), MIN(IPV6), MIN(VBIPV4), MIN(VBIPV6)
...
CREATE VIEW VP6 (TINY,
  ...
  SELECT TINY,
    MAX(ID), SUM(SMALL), COUNT(INT), MIN(BIG), MAX(NUM), SUM(DEC),
    MIN(VCHAR_INLINE), MIN(VCHAR_INLINE_MAX), MIN(VCHAR_OUTLINE_MIN), MIN(VCHAR), MIN(VCHAR_JSON), MIN(TIME)
    -- Will not work with SqlCoverage/PostgreSQL:
    , MIN(VARBIN), MIN(POINT), MIN(POLYGON)
    , MIN(IPV4), MIN(IPV6), MIN(VBIPV4), MIN(VBIPV6)
...
CREATE VIEW VR7 (SMALL, VCHAR, TINY,
  ...
  SELECT SMALL, VCHAR, TINY,
    MAX(ID), COUNT(TIME), COUNT(POINT), SUM(NUM), SUM(DEC),
    MIN(VCHAR_INLINE), MAX(VCHAR_INLINE_MAX), MIN(VCHAR_OUTLINE_MIN), MAX(VCHAR_JSON), MIN(TIME)
    -- Will not work with SqlCoverage/PostgreSQL:
    , MAX(VARBIN), MIN(POINT), MAX(POLYGON)
    , MIN(IPV4), MAX(IPV6), MIN(VBIPV4), MAX(VBIPV6)
...
CREATE INDEX IDX_VR7_VID ON VR7 (VCHAR_INLINE, INT, DEC)  WHERE VCHAR_INLINE < 'a';
...
CREATE INDEX IDX_VP7_VID ON VP7 (VCHAR_INLINE, INT, DEC)  WHERE VCHAR_INLINE < 'a';
