...
,   C11 GEOGRAPHY_POINT NOT NULL
,   C12 GEOGRAPHY NOT NULL
,   PRIMARY KEY
    (
        C1
    ,   C9
...
CREATE PROCEDURE UPS_T4_COLS_SELECT_T4 AS
    INSERT INTO T4 (C9, C1, C4, C5, C8, C6, C7, C11, C12)
        SELECT C9, C1, C4, C5, C8, C6, C7, C11, C12 FROM T4;
...
CREATE TABLE GEO (
    ...
    GEO   GEOGRAPHY,
    GEOPT GEOGRAPHY_POINT
);

-- This seems to fail.
-- CREATE INDEX GEOINDEX_ISVALID ON GEO ( ID, ISVALID(GEO) );
CREATE INDEX GEOINDEX_REASONS ON GEO ( ID, ISINVALIDREASON(GEO) );
-- geo functions
CREATE INDEX INDEX_USES_GEO_ASTEXT_POINT ON T4 (ASTEXT(C11));
CREATE INDEX INDEX_USES_GEO_ASTEXT_POLYGON ON T4 (ASTEXT(C12));
CREATE INDEX INDEX_USES_GEO_LONGITUDE ON T4 (LONGITUDE(C11));
CREATE INDEX INDEX_USES_GEO_LATITUDE ON T4 (LATITUDE(C11));
