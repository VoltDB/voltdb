...
function server() {
    ...
    VOLTDB_OPTS="${VOLTDB_OPTS}" ${VOLTDB} create
}
...
function run-benchmark-help() {
    srccompile
    java -classpath obj:$APPCLASSPATH:obj log4jbenchmark.Log4jBenchmark --help
}

function run-benchmark() {
    srccompile
    java -classpath obj:$APPCLASSPATH:obj -Dlog4j.configuration=file://$LOG4J \
        log4jbenchmark.Log4jBenchmark \
        --displayinterval=5 \
        --duration=30 \
        --servers=localhost \
...
function help() {
    echo "Usage: ./run.sh {clean|server|sync-benchmark|sync-benchmark-help|...}"
}
