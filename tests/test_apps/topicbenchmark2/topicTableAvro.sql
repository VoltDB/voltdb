...
CREATE TOPIC USING STREAM TEST_TOPIC EXECUTE PROCEDURE test_topic PROFILE topicbenchmark PROPERTIES (topic.format=avro,producer.parameters.includeKey=true,consumer.keys=rowid);
...
CREATE TOPIC USING STREAM TEST_TOPIC01 EXECUTE PROCEDURE test_topic01 PROFILE topicbenchmark PROPERTIES(topic.format=avro,producer.parameters.includeKey=true,consumer.keys=rowid);
