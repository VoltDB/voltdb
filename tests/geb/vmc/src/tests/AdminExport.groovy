...
class AdminExport extends TestBase {
	...
	def "Check export add configuration for empty values and validate errors" (){
		...
		then:
		waitFor(waitTime){	
		    page.addconfigpopup.isDisplayed()
		...
		if(page.addconfigtxt.text().equals("Add Configuration") && page.streamtxt.text().equals("Stream") && page.typetxt.text().equals("Type") && page.propertiestxt.text().equals("Properties") && page.nametxt.text().equals("Name") && page.valuetxt.text().equals("Value") && page.deletetxt.text().equals("Delete")) { 
		    ...
		    println("None of the title and label verified")
		...
		waitFor(waitTime){	
		    page.reqfielderror.isDisplayed()
		}
	...
	def "Verify export add configuration for given values from files and save in ON state" (){
		...
		waitFor(waitTime){	
		    page.addconfigpopup.isDisplayed()
			page.addconfigtxt.isDisplayed()
			if(page.streamtxt.isDisplayed()) {
			    page.inputstream.value(streamValue)
			}
			if(page.typetxt.isDisplayed()) {
			    page.inputtype.value(typeValue)
			}
			page.propertiestxt.isDisplayed()
			if(page.nametxt.isDisplayed()) {
			    page.inputnamefrst.value(nameValue)
			}
			if(page.valuetxt.isDisplayed())	{
			    page.inputvaluefrst.value(valueValue)
			}
			page.deletetxt.isDisplayed()
		}
									
		waitFor(waitTime){	
		    page.saveconfig.isDisplayed()
			page.cancelconfig.isDisplayed()
		}
		page.saveconfig.click()

		waitFor(waitTime){	
		    page.confirmpopupask.isDisplayed()
		...
		try{		
		    if(!page.exportbtn.isDisplayed() || !page.dbmonitorerrormsgpopup.isDisplayed()) {
		        page.confirmyesbtn.click()
	        }
		    else 
		        println("confirm pop up for save Yes clicked")
		...
		try{		
		    ...
		    println("Time exceed more than the given waiting time")
		}
		
		waitFor(waitTime) {
		    page.belowexportbtn.isDisplayed()
		}
		if(!page.belowexportbtn.isDisplayed()) {
		    page.exportbtn.click()
		}
		else
		...
		try{
		    waitFor(waitTime) {
		        page.addconfig.isDisplayed()
				if(page.onstatetxt.isDisplayed()) {
				    println("It is in ON STATE")
				}
				else 
				    println("It is in OFF STATE")				
				page.belowexportnametxt.isDisplayed()
				page.belowexportvaluetxt.isDisplayed()
			}
		} catch(geb.error.RequiredPageContentNotPresent e) {
		    println("element couldn't found")
		} catch(geb.waiting.WaitTimeoutException e) {
		    println("Took time due to condition mismatch")
		}
	...
	def "Verify export add configuration for given duplicate values of Stream from file and validating the errors" (){
		...
		waitFor(waitTime) {	
		    page.samestreamnameerrorpopup.isDisplayed()
    ...
    def "Verify export add configuration for given values from files and save in OFF state" () {
		...
		waitFor(waitTime) {	
			...
			page.addconfigtxt.isDisplayed()
			if(page.streamtxt.isDisplayed()) {
			    page.inputstream.value(streamValueScnd)
			}
			if(page.typetxt.isDisplayed()) {
			    page.inputtype.value(typeValue)
			}
		...
		try {	
		    waitFor(waitTime) {
		     	if(page.addconfigcheckonbox.value()=="on" && page.addconfigcheckonbox.isChecked()) {
		    ...
		    waitFor(waitTime) {	
		        if(page.checkboxofftxt.text()=="Off") {
		            println("OFF state")
		        }
		        else
        			page.checkboxtest.click()
		...
		try{
			...
			if(page.offstatetxt.isDisplayed()) {
			    println("It is in OFF STATE")
			} 
			else 
			    println("It is in ON STATE")						
	        ...
	        println("element content not found")
	    }
    }
	...
	def "Verify export EDIT configuration and add property from files, save it and delete property in ON state" () {	
    	...
    	try{		
	        waitFor(waitTime) {	
	   	        page.exporteditbtn.isDisplayed()
				page.onstatetxt.isDisplayed()
			}
			if(page.onstatetxt.text()=="On") {
		...
		try {
	       	waitFor(waitTime) {
	       	    if(!page.inputnamescnd.isDisplayed() && !page.inputvaluescnd.isDisplayed()) {
	       	        page.addproperty.click()
	       	    }
			}
		} catch (geb.error.RequiredPageContentNotPresent e) { 
		...
		waitFor(waitTime) {	
		    if(page.inputnamescnd.isDisplayed()) {
		        page.inputnamescnd.value(nameScnd)
		    }
			if(page.inputvaluescnd.isDisplayed()) {
			    page.inputvaluescnd.value(valueScnd)
			}
		}
	    ...
	    try{
	       	waitFor(waitTime) {	
	       	    page.addconfig.isDisplayed()
			    if(page.onstatetxt.isDisplayed() && page.onstatetxt.text()=="On")
			        println("It is in ON STATE")
			    else 
			        println("It is in OFF STATE")				
		...
		if(page.belowexportnamescndtxt.text().equals(nameScnd) && page.belowexportvaluescndtxt.text().equals(valueScnd)) {
    	    ...
    	    println("Value from the file did not matched")	
		...
		then:'check the updated results'	
       	try{
			if(waitFor(waitTime){!page.belowexportnametxt.isDisplayed()}) {
			    page.belowexportbtn.click()
			}									
		}catch(geb.waiting.WaitTimeoutException e) {
		    println("Took time for element to find.")
		}	
		...
		if(page.belowexportnametxt.text().equals(nameValue) && page.belowexportvaluetxt.text().equals(valueValue)) {
		    ...
		    println("updated file name and its value mis matched")
	}	
	...
	def "verify Delete Configuration for export setting in ON STATE"(){
		...
		waitFor(waitTime) {	
		    page.deleteconfigurations.isDisplayed()
		}
		...
		if (page.deleteconfirmation.text().equals("Are you sure you want to delete?")) {
		    println("delete confirmation verified in ON STATE")
		}
		...
		if(page.noconfigtxt.text()=="No configuration available.") {
		    println("No Configuration verified")
		}
	}
	...
	def "verify Delete Configuration for export setting in OFF STATE"() {
		...
		if(page.deleteconfirmation.text().equals("Are you sure you want to delete?")) {
		    println("delete confirmation verified in OFF STATE")
		}
