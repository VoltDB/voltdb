...
{_mostmath |= " / "}
SELECT _variable[numeric] _mostmath 0.9999 AS BIG_MATH FROM @fromtables
...
-- Substituting these two statements to cut down on the umber of generated combinations and limit math range errors:
   SELECT 3, * FROM @fromtables WHERE @optionalfn(_variable[numeric]) _cmp (           (_variable[numeric]) _math _value[int:1,10])
   SELECT 3, * FROM @fromtables WHERE            (_variable[numeric]) _cmp (@optionalfn(_variable[numeric]) _math _value[int:1,10])
...
-- ticket 232
SELECT _variable[#A numeric] FROM @fromtables GROUP BY __[#A] ORDER BY __[#A]
...
--SELECT @optionalfn(_variable[@columntype]) _math _value[float] FROM @fromtables
SELECT _numfun(_variable[numeric]) _math _numfun(_variable[numeric]) AS Q27 FROM @fromtables
