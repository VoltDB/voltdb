...
{_subqueryform |= "(select   X._variable[#float float]  __[#float],   X._variable[#arg numeric]  __[#arg], X._variable[#string string] __[#string] from _table[#self] X _jointype join __[#self] Y on X.ID = Y.ID where Y._numericcolumnpredicate)"}
