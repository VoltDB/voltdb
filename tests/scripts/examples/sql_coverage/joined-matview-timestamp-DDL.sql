...
CREATE VIEW V1 (ID) AS
  SELECT COUNT(*) FROM P1 T1 JOIN R1 T2 ON T1.ID = T2.PRESENT;

CREATE VIEW V2 (ID) AS
  SELECT COUNT(*) FROM R1 T1 JOIN R2 T2 ON T1.PRESENT = T2.PRESENT;
...
CREATE VIEW V9 (PAST, ID) AS
  SELECT   T1.PAST, COUNT(*) FROM R2 T1 JOIN P2 T2 ON T1.PRESENT = T2.ID
  GROUP BY T1.PAST;

CREATE VIEW V10 (ID, PRESENT) AS
...
CREATE VIEW V13 (FUTURE, PRESENT, ID) AS
  SELECT   T2.FUTURE, COUNT(*),   SUM(T1.ID)    FROM P1 T1 JOIN R1 T2 USING(ID)
  GROUP BY T2.FUTURE;

CREATE VIEW V14 (ID, PRESENT, PAST) AS
...
CREATE VIEW V17 (PAST, ID, FUTURE) AS
  SELECT   T1.PAST, COUNT(*),   SUM(T2.FUTURE) FROM R1 T1 JOIN R2 T2 ON T1.PRESENT = T2.FUTURE
  GROUP BY T1.PAST;
...
CREATE VIEW V22 (PAST, PRESENT, COUNT_STAR, ID, FUTURE) AS
  SELECT   T1.PAST, T2.PRESENT,   COUNT(*),   MAX(T2.ID),      SUM(T1.FUTURE) FROM R1 T1 JOIN R2 T2 ON T1.PRESENT = T2.PRESENT
  GROUP BY T1.PAST, T2.PRESENT;

CREATE VIEW V23 (PRESENT, PAST, PRESENT2, COUNT_STAR, ID, FUTURE, FUTURE2) AS
  SELECT   T1.PRESENT, T1.PAST, T2.PRESENT, COUNT(*), MIN(T2.ID), MAX(T1.FUTURE), SUM(T2.FUTURE) FROM P1 T1 JOIN R1 T2 ON T1.ID = T2.ID
...
CREATE VIEW V26 (ID, PRESENT, FUTURE) AS
  SELECT   T1.ID,    COUNT(*),   SUM(T2.FUTURE) FROM R1 T1 JOIN R2 T2 ON T1.PRESENT <= T2.PRESENT
  GROUP BY T1.ID;

CREATE VIEW V27 (PAST, ID, PRESENT) AS
...
CREATE VIEW V33 (FUTURE, COUNT_STAR, ID, PRESENT) AS
  SELECT   T1.FUTURE, COUNT(*), COUNT(T2.PAST),   SUM(T3.PRESENT)   FROM P1 T1 JOIN R1 T2 ON T1.ID = T2.ID JOIN R2 T3 ON T2.ID = T3.ID
  GROUP BY T1.FUTURE;

CREATE VIEW V34 (FUTURE, ID, PRESENT, PAST) AS
  SELECT   T1.FUTURE, COUNT(*),   SUM(T2.PRESENT),     MAX(T3.PAST) FROM R1 T1 JOIN R2 T2 ON T1.ID = T2.ID JOIN R3 T3 ON T2.ID = T3.ID
  GROUP BY T1.FUTURE;
