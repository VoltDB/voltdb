...
SELECT A.ID AID, B.ID BID, DISTANCE(A.PT1  ,B.PT1  ) G13DIST FROM _table A JOIN _table B ON A.ID <= B.ID
SELECT ID, DISTANCE(PT1,  POLY1) from _table G14
SELECT ID, DISTANCE(PT1,  POLY1) from _table G15 WHERE DISTANCE(PT1,  POLY1) > 200000
SELECT A.ID AID, B.ID BID, DISTANCE(A.PT1  ,B.POLY1) G16DIST FROM _table A JOIN _table B ON A.ID <= B.ID
SELECT ID, DISTANCE(POLY1,PT1  ) from _table G17
SELECT ID, DISTANCE(POLY1,PT1  ) from _table G18 WHERE DISTANCE(POLY1,PT2  ) > 200000
SELECT A.ID AID, B.ID BID, DISTANCE(A.POLY1,B.PT1  ) G19DIST FROM _table A JOIN _table B ON A.ID <= B.ID
-- DISTANCE between two polygons is not yet supported
SELECT ID, DISTANCE(POLY1,POLY1) from _table G20
SELECT ID, DISTANCE(POLY1,POLY1) from _table G21 WHERE DISTANCE(POLY1,POLY1) < 200000
SELECT A.ID AID, B.ID BID, DISTANCE(A.POLY1,B.POLY1) G22DIST FROM _table A JOIN _table B ON A.ID <= B.ID

-- Test polygons and the asText, AREA & CENTROID functions (also using LONGITUDE, LATITUDE)
SELECT ID,              asText(POLY1) from _table G31
SELECT ID, AREA(POLY1)                from _table G32
SELECT ID, AREA(POLY1)                from _table G33 WHERE AREA(POLY1) > 2000000
SELECT ID, AREA(POLY1), asText(POLY1) from _table G34
SELECT ID, AREA(POLY1), asText(POLY1) from _table G35 WHERE AREA(POLY1) > 2000000
SELECT ID, asText(CENTROID(POLY1))    from _table G36
SELECT ID, LONGITUDE(PT1), LATITUDE(PT1), LONGITUDE(PT2), LATITUDE(PT2), LONGITUDE(CENTROID(POLY1)), LATITUDE(CENTROID(POLY1)) from _table G37
