...
static int BN_ext_count_low_zero_bits(const BIGNUM* bn) {
   ...
   DCHECK_EQ(BN_bn2lebinpad(bn, bytes.data(), bytes.size()), bytes.size());
...
ExactFloat::ExactFloat(double v) : bn_(BN_new()) {
...
ExactFloat::ExactFloat(int v): bn_(BN_new()) {
  sign_ = (v >= 0) ? 1 : -1;
