...
public class ReadOnlySlow extends VoltSystemProcedure {
    ...
    void activateCopyOnWriteTables(SystemProcedureExecutionContext ctx, CorePlan plan) {
        ...
        JSONObject obj;
        try {
            obj = new JSONObject(new String(plan.aggregatorFragment));
    ...
    public VoltTable[] run(SystemProcedureExecutionContext ctx,
        ...
        for (AdHocPlannedStatement statement : statements) {
            if (!statement.core.wasPlannedAgainstHash(ctx.getCatalogHash())) {
                ...
                String msg = String.format("ReadOnlySlow transaction %d wasn't planned " +
            ...
            // Must be read only
            if (!statement.core.readOnly) {
                String msg = String.format("ReadOnlySlow only supports read only queries. Statement: %s",
                        new String(statement.sql, Constants.UTF8ENCODING));
                throw new VoltAbortException(msg);
            }
