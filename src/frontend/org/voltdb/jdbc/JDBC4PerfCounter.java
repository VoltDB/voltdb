 ...
 * @since 2.0
 */
public class JDBC4PerfCounter implements Cloneable {
     ...
     *
     * The first 100 buckets cover 0 - 100ms, 1ms each (e.g. (0, 1ms]). The next 8 buckets cover 100
     * - 500ms, 50ms each. The last one covers 500ms+.
     */
    private final long[] lat = new long[109];

    /**
     * Creates a new performance counter and immediately starts tracking time (for rate/second
     * calculations).
     */
    public JDBC4PerfCounter() {
        this(true);
    }
     ...
     *
     * @param start
     *            the flag indicating whether time tracking should be started immediately. When
     *            false time tracking is started on the first counter update.
     */
    public JDBC4PerfCounter(boolean start) {
        if (start)
            this.StartTime = System.currentTimeMillis();
    }

    /**
     * Gets the time (in milliseconds since 1/1/1970 00:00 UTC) at which this counter's time
     * tracking began (the time at which this counter was created - if created with the start
     * parameter set to true - or the time of the first update).
     */
    ...
    /**
     * Gets the time (in milliseconds since 1/1/1970 00:00 UTC) at which this counter's time
     * tracking ended (the time if the last update).
     */
    ...
    /**
     * Gets the total execution duration tracked by this counter (the sum of execution durations of
     * all calls tracked by this counter).
     */
    ...
    /**
     * Gets the average number of execution calls per second for all execution calls tracked by this
     * counter.
     */
     ...
     *
     * @param response
     *            the response sent by the VoltDB server, containing details about the
     *            procedure/statement execution.
     */
    ...
    /**
     * Tracks a generic call execution by reporting the execution duration. This method should be
     * used for successful calls only.
     *
     * @param executionDuration
     *            the duration of the execution call to track in this counter.
     ...
     *            the duration of the execution call to track in this counter.
     * @param success
     *            the flag indicating whether the execution call was successful.
     */
    ...
    /**
     * Gets a representation of this counter as a single-line short-format string detailing the
     * statistics tracked by this counter.
     *
     ...
     *            the flag indicating whether to use a short one-line format, or detailed statistics
     *            including latency bucketing.
     * @return the string representation of this counter.
     */
    public String toString(boolean useSimpleFormat) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
        dateFormat.setTimeZone(TimeZone.getTimeZone("UTC"));
        long elapsedDuration = (this.StartTime == Long.MAX_VALUE) ? 1 : this.EndTime
                - this.StartTime;
        if (useSimpleFormat)
            return String.format("%8s | Txn.: %,11d%s @ %,11.1f TPS | Lat. = %7d <  %7.2f < %7d\n",
                    ...
                    this.cnt, this.err > 0 ? String.format(" [!%,11d]", this.err) : "",
                    (this.cnt * 1000d / (double) elapsedDuration), this.min == 999999999l ? 0l
                            : this.min, (double) this.tot / (double) this.cnt, this.max == -1l ? 0l
                            : this.max);
        else {
            ...
            return String
                    .format("-------------------------------------------------------------------------------------\n"
                            ...
                            this.cnt, this.err > 0 ? String.format(" [!%,11d]", this.err) : "",
                            (this.cnt * 1000d / elapsedDuration), this.min == 999999999l ? 0l
                                    : this.min, (double) this.tot / (double) this.cnt,
                            this.max == -1l ? 0l : this.max,
                            100 * (double) coarseLat[0] / this.cnt, 100 * (double) coarseLat[1]
                                    / this.cnt, 100 * (double) coarseLat[2] / this.cnt, 100
                                    * (double) coarseLat[3] / this.cnt, 100 * (double) coarseLat[4]
                                    / this.cnt, 100 * (double) coarseLat[5] / this.cnt, 100
                                    * (double) coarseLat[6] / this.cnt, 100 * (double) coarseLat[7]
                                    / this.cnt, 100 * (double) coarseLat[8] / this.cnt);
        }
    }
    ...
    public String toRawString(char delimiter) {
        ...
        sb.append(getStartTime()).append(delimiter).append(getEndTime()).append(delimiter)
                .append(getExecutionCount()).append(delimiter).append(getMinLatency())
                .append(delimiter).append(getMaxLatency());
     ...
     */
    @Override
    public JDBC4PerfCounter clone() {
        JDBC4PerfCounter counter = new JDBC4PerfCounter(false);
     ...
     *
     * @param counters
     *            the list of counters to merge statistics from.
     * @return the new counter containing aggregated statistics from all the provided counters.
     */
    public static JDBC4PerfCounter merge(JDBC4PerfCounter[] counters) {
        JDBC4PerfCounter counter = counters[0].clone();
    ...
    /**
     * Performs a difference between the statistics of this counter and those of a previously cloned
     * counter, for incremental statistics tracking.
     *
     * @param previous
     *            the counter to compare statistics with to generate a difference.
     * @return the new counter containing the incremental statistics between the two counters. The
     *         original counter is left unchanged.
     */
    public JDBC4PerfCounter difference(JDBC4PerfCounter previous) {
        JDBC4PerfCounter diff = this.clone();
        if (previous != null) {
