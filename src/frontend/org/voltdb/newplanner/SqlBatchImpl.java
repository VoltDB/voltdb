...
import org.apache.commons_voltpatches.cli.ParseException;
...
public class SqlBatchImpl implements SqlBatch {
    ...
    public SqlBatchImpl(String sqlBlock, Object... userParams)
            throws SqlParseException, PlannerFallbackException, ParseException {
        ...
        for (String sql : sqlList) {
            try {
                ...
                String msg = "Error: invalid SQL statement in line: " + (lineNo + e.getPos().getLineNum()) + ", column: " + e.getPos().getColumnNum() + ". " +
                             "Expecting one of: " + e.getExpectedTokenNames();
                log.debug(msg);
                throw new PlannerFallbackException(msg);
                // TODO throw a parse exception instead of fallback exception to reflect error position
                // respond error message back to sqlcmd
            }
