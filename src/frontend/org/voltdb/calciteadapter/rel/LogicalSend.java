...
public class LogicalSend extends SingleRel {

    private VoltDBPartitioning m_partitioning;
    // With each LogicalSend pull up the LEVEL must be incremented for Calcite to be able to differentiate
    // between the two send node - before and after the transformation
    private int m_level;

    public LogicalSend(RelOptCluster cluster, RelTraitSet traitSet,
...
//    }
    ...
    @Override
    public LogicalSend copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new LogicalSend(getCluster(), traitSet, sole(inputs), m_partitioning, m_level);
      }

    public RelNode copy(RelNode input, int level) {
        return new LogicalSend(getCluster(), getTraitSet(), input, m_partitioning, level);
    }
