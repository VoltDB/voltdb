...
public class VoltDBLogicalAggregateRule extends RelOptRule {

        public static final VoltDBLogicalAggregateRule INSTANCE = new VoltDBLogicalAggregateRule();

        VoltDBLogicalAggregateRule() {
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            RelTraitSet convertedTraits = aggr.getTraitSet().replace(VoltDBLogicalRel.VOLTDB_LOGICAL);
            RelNode convertedInput = convert(input, input.getTraitSet().replace(VoltDBLogicalRel.VOLTDB_LOGICAL));
            call.transformTo(VoltDBLogicalAggregate.create(
                    aggr.getCluster(),
                    convertedTraits,
                    convertedInput,
                    aggr.indicator,
