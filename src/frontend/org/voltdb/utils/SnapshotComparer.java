...
class SnapshotLoader {
    ...
    public SnapshotLoader(boolean local, String username, String nonce, String[] dirs, String[] hosts, String[] verifyTables) {
        ...
        catalog.execute(CatalogUtil.getSerializedCatalogStringFromJar(jar));
        for (Table table : getTables(CatalogUtil.getDatabase(catalog))) {
            String name = table.getTypeName();

            if (verifyTables == null) {
                tables.put(name, table.getIsreplicated());
            } else if (tableSet.contains(name.toUpperCase())) {
                tables.put(name, table.getIsreplicated());
                tableSet.remove(name.toUpperCase());
            }
        }
            ...
            for (ChannelSftp.LsEntry item : fileAndFolderList) {
                if (!item.getAttrs().isDir()) { // Check if it is a file (not a directory).
                    if (!(new File(destinationPath + PATHSEPARATOR + item.getFilename())).exists() ||
                            (item.getAttrs().getMTime() > Long.valueOf(new File(destinationPath + PATHSEPARATOR + item.getFilename()).lastModified() / 1000).intValue())) { // Download only if changed later.
    ...
    private static List<Table> getTables(Database database) {
        List<Table> tables = new ArrayList<>();

        assert database != null;

        tables.addAll(CatalogUtil.getNormalTables(database, true));
        tables.addAll(CatalogUtil.getNormalTables(database, false));
        return tables;
    }
}
