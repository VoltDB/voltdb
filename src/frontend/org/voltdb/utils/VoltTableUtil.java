...
public class VoltTableUtil {
    ...
    private static final VoltTable.ColumnInfo m_compressedTableColumns[] =
            new VoltTable.ColumnInfo[] { new VoltTable.ColumnInfo("", VoltType.VARBINARY) };

    public static VoltTable toCompressedTable(VoltTable table) {
        VoltTable retval = new VoltTable(m_compressedTableColumns);
        try {
            retval.addRow(table.getCompressedBytes());
        } catch (IOException e) {
    ...
    public static VoltTable toCompressedTable(VoltTable tables[]) {
        VoltTable retval = new VoltTable(m_compressedTableColumns);
        try {
            for (VoltTable vt : tables) {
                retval.addRow(vt.getCompressedBytes());
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public static VoltTable uncompressTable(VoltTable vt) {
        if (!vt.advanceRow()) {
        ...
        try {
            return PrivateVoltTableFactory.createVoltTableFromBuffer(
                    ByteBuffer.wrap(org.xerial.snappy.Snappy.uncompress(compressedTableBytes)),
                    false);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public static VoltTable[] uncompressTables(VoltTable vt) {
        VoltTable out[] = new VoltTable[vt.getRowCount()];
        for (int ii = 0; ii < out.length; ii++) {
            out[ii] = uncompressTable(vt);
        }
        return out;
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        VoltTable compressed = toCompressedTable(vt);
        ...
        VoltTable uncompressed = uncompressTables(compressed2)[0];
