...
function alertNodeClicked(obj) {
    ...
    if ($('#activeServerName').html() != clickedServer) {
        ...
        //TODO: Plot graph for the selected server
...
(function (window) {
    ...
    var iVoltDbRenderer = (function () {
        ...
        var testConnection = function(username, password, admin, onInformationLoaded) {
            VoltDBService.TestConnection(username, password, admin, function(result) {
                ...
                '<div id="loginBoxDialogue" style="overflow: hidden" >' +
                    '<div class="overlay-title">Login</div>' +
                        '<div id="UnableToLoginMsg" style="padding: 5px 0 0 20px; color: #ff0000; display: none;">Unable to connect!! Please try to login using another username/password.</div>' +
                            '<div class="clear"></div>' +
                            '<div  class="overlay-content" style="height:212px; min-width: 441px; padding: 0" >' +
                            '<div id="loginBox">' +
        ...
        this.HandleLogin = function (pageLoadCallback) {
            ...
            $("#loginLink").popup({
                ...
                login: function (popupCallback) {
                    ...
                    testConnection(usernameVal, passwordVal, false, function (result) {
            ...
            testConnection(username, password, true, function (result) {
        ...
        this.getDatabaseInformation = function (onInformationLoaded) {
            ...
            VoltDBService.GetSystemInformationDeployment(function (connection) {
                ...
                VoltDBService.GetDataTablesInformation(function (nestConnection) {
                    ...
                    onInformationLoaded(procedureMetadata, nestConnection.Metadata['@Statistics_TABLE'].data);
                });
            });
        ...
        this.getCpuGraphInformation = function (onInformationLoaded) {
            ...
            VoltDBService.GetCPUInformation(function (connection) {
                getCpuDetails(connection, cpuDetails, "GRAPH_CPU");
        ...
        this.GetTransactionInformation = function (onInformationLoaded) {
            ...
            VoltDBService.GetTransactionInformation(function (connection) {
                getTransactionDetails(connection, transactionDetails); //, "GRAPH_TRANSACTION");
        ...
        var populateProceduresInformation = function (connection) {
           ...
           //connection.Metadata['@Statistics_PROCEDUREPROFILE'] = GetTestProcedureData(connection);
            ...
            connection.Metadata['@Statistics_PROCEDUREPROFILE'].schema.forEach(function (columnInfo) {
                ...
                DbSchema['procedures'][counter] = columnInfo["name"];
                counter++;
        ...
        var populateTableTypes = function (connection) {
            ...
            connection.Metadata['@SystemCatalog_TABLES'].data.forEach(function (entry) {
                ...
                if (!schemaCatalogTableTypes.hasOwnProperty(tableName)) {
                    ...
                    schemaCatalogTableTypes[tableName]['TYPE_NAME'] = entry[tableTypeIndex];
        ...
        //this.mapProcedureInformation = function (isNext, isSearch, callback) {
        //    var counter = 0;
        //    var pageStartIndex = 0;
        //    var traverse = false;

        //    //if (isProcedureSearch) {
        //    //    //if (this.searchData.procedures.length == 0) //no tuple found as of search result, then empty htmlMarkup
        ...
        //    if ((((voltDbRenderer.procedureTableIndex + 1) * this.maxVisibleRows < voltDbRenderer.procedureDataSize) && isNext == true) || (!isNext && voltDbRenderer.procedureTableIndex > 0) || isNext == undefined) {
        //        if (isNext == true) {
        //            pageStartIndex = (voltDbRenderer.procedureTableIndex + 1) * voltDbRenderer.maxVisibleRows;


        //        }
        //        else if (isNext == false) { // pageStartIndex need not be initialized if isNext is undefined(when page loads intially or during reload operation)
        //            pageStartIndex = (voltDbRenderer.procedureTableIndex - 1) * voltDbRenderer.maxVisibleRows;
        //        }


        //        var lProcedureData = this.isProcedureSearch ? this.searchData.procedures : procedureData;
        ...
        //        if (this.isProcedureSearch)
        //            callback(traverse, htmlMarkup);

        //        else {
        //            htmlMarkups.SystemInformation.push(htmlMarkup);
        //            htmlMarkup = undefined;
        //            callback(traverse, htmlMarkups);
        ...
        //TODO: Render CPU Graph
        var getCpuDetails = function (connection, sysMemory, processName) {
            ...
            connection.Metadata['@Statistics_CPU' + suffix].schema.forEach(function (columnInfo) {
