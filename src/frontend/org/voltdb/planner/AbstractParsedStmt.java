...
public abstract class AbstractParsedStmt {
  ...
  private AbstractExpression parseRowExpression(List<VoltXMLElement> exprNodes) {
     ...
     * <p>
     * The set of order-by expressions is a set, so it doesn't have an order.
     * But each expression in it comes from a list of columns, so there is
     * an implicit order. Let's set some notation.  Our goal in the sequence
     * of definitions is to define an <em>honorary order-by expression</em>,
     * but getting there requires some work.
     * <p>
     * <ol>
     *   <li>Queries may have limits and offsets.  These just clutter up the
     *       argument, so we ignore them here.</li>
     *   <li>Let R be the relation formed by doing the joins and where-filtering,
     *       and perhaps grouping, but before ordering and selecting.  The columns
     *       of R are the columns of the tables in the FROM list, or else the
     *       columns defined by the group-by expressions.  We assume that elements
     *       of R all have unique indices drawn from a finite index set I.  We
     *       can refer to elements of R as ri, rj and so forth.  The set I is not
     *       necessarily ordered.  <b>Note</b> that are depends on the state of
     *       the database tables.  So, in some sense it's an input variable.</li>
     *   <li>Let the select list be C1, ..., Cn, let CE1, ..., CEn
     *       be the list of expressions of the columns and let CN1, ..., CNn be
     *       the names of the columns.  These
     *       CEks are expressions in the columns of R.  If there is a
     *       group-by clause, the CEks must be factorable as CEk = CETk(GE1, ..., GEl),
     *       where each GEk is a group-by expression.  We say this factorization
     *       makes the select expressions <em>possibly grouped.</em>
     *       (This is actually stronger
     *       than the true requirement, but this will do for the purposes of this
     *       note.)</li>
     *   <li>Let the original list of order-by expressions be E = E1, ..., Em.  Then
     *       the Eks are functions of the columns of the
     *       select list CE1, possibly grouped.</li>
     *   <li>Given a list of expressions, E, and an expression, H, we write
     *       E+H for the list E with H added to the end.  So, if E = E1, ..., Em, then
     *       E+H = E1, ..., Em, H.</li>
     *   <li>For each row of R,
     *       say ri, we can calculate a tuple of values ce=[ce1, ..., cen] using the values
     *       in row ri.  We then sort the set of ce tuples.  Since each such ce-tuple
     *       is associated with an index, i, this sorting induces a partial ordering, [E[, on
     *       I, the index set of R.
     *       <ul>
     *         <li>We include the set of order-by expressions, E,
     *             in the notation because the ordering relation depends on it.</li>
     *         <li>We use square brackets for ordering to avoid needing HTML entities
     *             in the JavaDoc.)</li>
     *       </ul>
     *       Note that [E[ is only a partial order.  It may be that two rows, say ri and
     *       rj, both have the same tuple value.  In this case i and j are incomparable.</li>
     *   <li>Let SS be the set of all ce tuples.</li>
     *   <li>We say a query is <em>order deterministic</em> iff [E[ is a total order on SS.</li>
     *   <li>An expression, H, is an <em>honorary order-by expression</em> iff [E[ as a
     *       partial order on SS is identical to [E+H[ on SS for every possible R.</li>
     * </ol>
     * <p>
     * Our problem, then, is to figure out which expressions are honorary order-by expressions,
     * There are many of them, and it would be useless to try to enumerate them all.  (Infinity
     * is a kind of many.)  So, we want to add honorary order-by expressions to the orderByExprs
     * list which are useful in determining order determinism.
     * <p>
     * The ordering relation [E[ is generated by evaluations of expressions in E.  For each
     * Ei in E = E1, ..., En let domain of Ei, denoted [[Ei]] be the set of values of the type
     * of Ei.  That is, if Ei is an integer expression, let [[Ei]] be the set of integers, and
     * the same for other types.  Then  let the domain of E, denoted [[E]] be [[E1]] X [[E2]] X ... X [[En]],
     * the cartesian product of the domains of the Eis.  The ordering relation [E[ is generated
     * by the function Eval(E) : I --> [[E]] which sends the index of each row in R to the evaluations of the
     * order-by expressions.  Similarly, Eval(E+H) : I -->[[E]]X[[H]] is Eval(E) X Eval(H) which
     * sends the index i into the tuple whose initial elements are Eval(E) and whose final
     * element is Eval(H).  There are two cases.
     * <ol>
     *   <li>If H is constrained to be a constant, so that Eval(H) is always a single value,
     *       then a tuple [e1, ..., en, h] [E[ [f1, ..., fn, h] iff [e1, ..., en] [E[ [f1, ..., fn].
     *       So, the induced ordering must be the same, and H is an honorary order-by expression
     *       of E.  Such a constraint can occur in a WHERE clause.</li>
     *   <li>If an expression, G, is constrained to be a constant, and H is constrained to
     *       be equal to G, then H is an honorary order-by expression, for similar reasons.
     *       Similarly, for all expressions in the transitive closure of such constrained
     *       equalities.  Not all of these are helpful.</li>
     *   <li>Let's consider unique indices.  Each row, ri, of R can be written as ri = [ri1, ..., rin], where rik is
     *       a tuple from table scan TSk.  Each table scan TSk has a a table, Tk, and
     *       an alias, TAk.  Note that Tk and Tl may be the same table for k != l, but
     *       TAk != TAl if k != l.  The table scan may have a table name, or the table may be a subquery,
     *       which has only an alias.  Let table Tk have an index constraint IX, on expressions
     *       IXE1, ..., IXEm.  Then each IXEk must be an expression on columns in
     *       table Tk.  Let C be a column in Tk.
     * </ol>
     */
