...
{
    if (sync) return;
    int64_t rowTarget = ExecutorContext::getExecutorContext()->getPhysicalTopend()->pushDRBuffer(m_partitionId, block);
    if (rowTarget >= 0) {
...
void AbstractDRTupleStream::fatalDRErrorWithPoisonPill(int64_t spHandle, int64_t uniqueId, const char *format, ...)
{
    ...
    ExecutorContext::getExecutorContext()->getPhysicalTopend()->pushPoisonPill(m_partitionId, failureMessageForVoltLogger, m_currBlock);
    m_currBlock = NULL;
    if (m_opened) {
