...
void MaterializedViewMetadata::processTupleInsert(const TableTuple &newTuple, bool fallible)
{
    ...
    if (exists) {
        ...
        m_updatedTuple.setNValue((int)m_groupByColumnCount,
                                 m_existingTuple.getNValue((int)m_groupByColumnCount).incrementBigInt());
        ...
        for (int aggIndex = 0; aggIndex < m_aggColumnCount; aggIndex++) {
            ...
            if (newValue.isNull()) {
                ...
                case EXPRESSION_TYPE_AGGREGATE_SUM:
                    ...
                    break;
                case EXPRESSION_TYPE_AGGREGATE_COUNT:
                    newValue = existingValue.incrementBigInt();
                    break;
                case EXPRESSION_TYPE_AGGREGATE_MIN:
...
void MaterializedViewMetadata::processTupleDelete(const TableTuple &oldTuple, bool fallible)
{
    ...
    NValue count = m_existingTuple.getNValue((int)m_groupByColumnCount).decrementBigInt();
    ...
    for (int aggIndex = 0; aggIndex < m_aggColumnCount; aggIndex++) {
        ...
        if ( ! oldValue.isNull()) {
            ...
            case EXPRESSION_TYPE_AGGREGATE_SUM:
                ...
                break;
            case EXPRESSION_TYPE_AGGREGATE_COUNT:
                newValue = oldValue.isNull() ? existingValue : existingValue.decrementBigInt();
                break;
            case EXPRESSION_TYPE_AGGREGATE_MIN:
                reversedForMin = -1;
                // fall through...
