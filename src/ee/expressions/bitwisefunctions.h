...
template<> inline NValue NValue::callUnary<FUNC_VOLT_BITNOT>() const {
    if (getValueType() != ValueType::VALUE_TYPE_BIGINT) {
...
template<> inline NValue NValue::call<FUNC_BITAND>(const std::vector<NValue>& arguments) {
    ...
    if (lval.getValueType() != ValueType::VALUE_TYPE_BIGINT || rval.getValueType() != ValueType::VALUE_TYPE_BIGINT) {
...
template<> inline NValue NValue::call<FUNC_VOLT_BIT_SHIFT_LEFT>(const std::vector<NValue>& arguments) {
    vassert(arguments.size() == 2);
    const NValue& lval = arguments[0];
    if (lval.getValueType() != ValueType::VALUE_TYPE_BIGINT) {
...
template<> inline NValue NValue::callUnary<FUNC_INET6_ATON>() const {
    if (getValueType() != ValueType::VALUE_TYPE_VARCHAR) {
        ...
        return getNullValue(ValueType::VALUE_TYPE_VARBINARY);
    }
    ...
    return NValue::getAllocatedValue(ValueType::VALUE_TYPE_VARBINARY, (const char*) &addr, sizeof(addr), getTempStringPool());
}
